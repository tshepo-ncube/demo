      ...
      <div className="bg-white text-black rounded-3xl shadow-xl w-full overflow-hidden" style={{ maxWidth: "1300px" }}>
        <div className="md:flex w-full">
          ...
          <div className="w-full md:w-1/2 py-10 px-10">
            ...
            {error && <p className="error-message text-red text-center">{error}</p>}
            {showNext ? (
              <button onClick={handleRegistration} className="block w-full bg-openbox-green hover:bg-hover-obgreen focus:bg-hover-obgreen text-white rounded-lg px-3 py-3 font-semibold">Register</button>
            ) : (
              <button onClick={() => setShowNext(true)} className="block w-full bg-openbox-green hover:bg-hover-obgreen focus:bg-hover-obgreen text-white rounded-lg px-3 py-3 font-semibold">Next</button>
            )}
            <p className="text-center mt-3">Already have an account? <span className="text-hover-obgreen cursor-pointer" onClick={() => router.push("/")}>Login here</span></p>
          </div>
        </div>
      </div>
    </div>
